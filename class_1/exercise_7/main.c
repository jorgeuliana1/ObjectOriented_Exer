...
int _compara_nomes(const void *alunoA, const void *alunoB) {
    tAluno **a = (tAluno **)alunoA;
    tAluno **b = (tAluno **)alunoB;

    return strcmp(NomeAluno(*a), NomeAluno(*b));
}
...
void ImprimeAlunosAcima(tAluno **vetorDeAlunos, int numElems) {
    ...
    fprintf(csv, "Nome,Nota,Situacao\n");

    qsort(vetorDeAlunos, numElems, sizeof(tAluno*), _compara_nomes);

    for(i = 0; i < numElems; i++)
        if(NotaAluno(vetorDeAlunos[i]) > MEDIA)
            fprintf(csv, "%s,%.2f,Aprovado\n", NomeAluno(vetorDeAlunos[i]), NotaAluno(vetorDeAlunos[i]));
        else
            fprintf(csv, "%s,%.2f,Prova Final\n", NomeAluno(vetorDeAlunos[i]), NotaAluno(vetorDeAlunos[i]));
